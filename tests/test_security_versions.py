...
def test_requirements_have_secure_versions():
    """Test that requirements.txt specifies secure versions of vulnerable packages."""
    with open("requirements.txt", "r") as f:
        requirements = f.read()

    # Check Flask-CORS version (should be 4.0.2+ to fix PYSEC-2024-71 and GHSA-84pr-m4jr-85g5)
    ...
    assert parse_version(flask_cors_version) >= parse_version("4.0.2"), \
        f"Flask-CORS version {flask_cors_version} is vulnerable, should be 4.0.2+"
    ...
    assert parse_version(requests_version) >= parse_version("2.32.0"), \
        f"requests version {requests_version} is vulnerable, should be 2.32.0+"
    ...
    assert parse_version(black_version) >= parse_version("24.3.0"), \
        f"black version {black_version} is vulnerable, should be 24.3.0+"


def test_no_vulnerable_versions():
    """Test that requirements.txt does not contain known vulnerable versions."""
    with open("requirements.txt", "r") as f:
        requirements = f.read()
